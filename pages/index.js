...
const fetchData = async () => { setLoading(true); const res = await fetch('/api/data'); const json = await res.json(); setData(json); setLoading(false); };
...
const handleUpload = async () => { const formData = new FormData(); for (let file of files) { formData.append("files", file); } await fetch("/api/upload", { method: "POST", body: formData, }); fetchData(); };

return ( <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4"> <h1 className="text-2xl font-bold col-span-full">Tableau de bord client</h1>
